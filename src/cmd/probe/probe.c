...
"[-?\n@(#)probe (AT&T Labs Research) 2000-01-11\n]"
USAGE_LICENSE
...
"	such changes require manual intervention. A probe information file"
"	(see \b--key\b) with write mode enabled signifies that manual changes"
"	have been made and disables automatic regeneration"
"	(see \b--generate\b).]"
...
main(int argc, char** argv)
{
	...
	if ((options & TEST) || !(ps.st_mode & S_IWUSR))
	{
		...
		base = path + strlen(path) - BASE_MAX;
		strcpy(key, base);
		strcpy(base, "../../");
		strcpy(base + 6, probe);
		if (stat(path, &vs))
