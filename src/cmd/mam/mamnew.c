...
initdir(register char* s, char* suf)
{
	...
	if (!(d->attributes & A_directory))
	{
		...
		else if (n > state.ancestor) state.ancestor = n;
	}
	return(d);
}
...
ccflags(register char* s, struct var* v)
{
	...
	for (;;)
	{
		...
		*t++ = 0;
		...
		case 'I':
			pathcanon(s += 2, 0);
			if (s[0] && (s[0] != '.' || s[1]) && !omitted(s)) initdir(s, ".h");
			break;
		case 'O':
			if (!*(s + 2)) break;
			/*FALLTHROUGH*/
...
initrule(const char* as, char* ar, void* handle)
{
	...
	if (s >= r->name + 2 && *(s -= 2) == '.') x->suffix = s + 1;
	else s = "";
	if (r->action || !(t = strrchr(r->name, '/'))) x->base = r->name;
...
dumpvar(void)
{
	...
	if (v = getvar(state.mam->main, "CCFLAGS")) ccflags(v->value, v);
}
...
main(int argc, char** argv)
{
	...
	while (c = optget(argv, "d#[debug]x:[omit-dir]")) switch (c)
	{
	case 'd':
