...
#ifdef MAM
  must_make = force_flag && file->deps || noexist;
#else
  must_make = noexist;
#endif /*MAM*/
...
#ifdef MAM
      d->changed |= force_flag && file->deps || noexist || d_mtime > this_mtime;
#else
     ...
     struct file *file;
{
  if (file->cmds == 0)
	...
	file->update_status = 0;
#ifdef MAM
      else if (file->is_target || force_flag && file->deps)
#else
...
#define LIBDIR "."
...
#else
  char *buf = xmalloc (40 + strlen (libname) + 4 + 2 + 1);
#endif
  char *file, **dp;
...
#if _PACKAGE_ast
  {
    char *temp;
#ifndef _AMIGA
    sprintf (buf, "lib/lib%s.a", libname);
#else
    sprintf (buf, "lib/%s.lib", libname);
#endif
    temp = pathpath ((char*)0, buf, "", PATH_REGULAR);
    if (temp != 0)
      {
        mtime = name_mtime (temp);
        if (mtime != (time_t) -1)
	  {
	    *lib = temp;
	    if (mtime_ptr != 0)
