...
extern char *end_of_token_w32 PARAMS ((char*, char));
#endif
     ...
     int flags;
{
  ...
  int makefile_errno;
#if defined (WINDOWS32) || defined (__MSDOS__) || defined (_WIN32)
  int check_again;
#endif
...
#define record_waiting_files()						      \
  ...
  if (infile == 0 && (flags & RM_INCLUDED) && *filename != '/')
...
#if _PACKAGE_ast
      char *temp;
      temp = concat ("include", "/", filename);
      name = pathpath ((char*)0, temp, "", PATH_REGULAR);
      free (temp);
      if (name != 0)
	filename = name;
      else
	...
	{
	  name = concat (include_directories[i], "/", filename);
