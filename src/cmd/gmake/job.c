...
#ifdef MAM
extern int start_remote_job PARAMS ((char *file, char* command, char **argv,
		char **envp, int stdin_fd, int *is_remote, int *id_ptr,
		int *used_stdin, int silent, int recurse));
#else
...
#ifdef MAM
  if (just_print_flag || (child->remote <= 1 && !(flags & COMMANDS_SILENT) && !silent_flag))
    {
      flags |= COMMANDS_SILENT;
      if (!mam.out || mam.dynamic && mam.out != stdout)
        message (0, "%s", p);
    }
  else
    flags &= ~COMMANDS_SILENT;
...
#ifdef MAM
      if (start_remote_job (child->file->name, p,
