...
/* Nonzero means force all targets to be out of date (-F).  */
...
static const struct command_switch switches[] =
  {
...
#ifdef MAM
    { 'F', flag, (char *) &force_flag, 1, 1, 1, 0, 0,
	"force", 0,
	...
	"-b" },
#ifdef MAM
    { 'M', string, (char *) &mam_options, 1, 1, 1, 0, 0,
	"mam", "TYPE[:FILE[:[LABEL][:ROOT]]]",
...
#ifdef MAM
    { 'N', flag, (char *) &never_flag, 1, 1, 1, 0, 0,
	"really-just-print", 0,
	"Don't even run $(MAKE) or + commands" },
#endif /*MAM*/
...
#undef	FATAL_SIG
...
#ifdef MAM
  /* Set a few variables that are otherwise difficult to define.  */
  (void) define_variable ("newline", 7, "\n", o_override, 0);
  (void) define_variable ("sharp", 5, "#", o_override, 0);
#endif /*MAM*/
  ...
  if (mam_options)
	...
	*o++ = 0;
	...
	fatal ("Invalid mam type `%s': {dynamic,regress,static} expected", s);
      while (s = o)
        {
          if (o = strchr(s, ','))
	    *o++ = 0;
	...
	{
	  mam.out = fopen (s, mam.parent ? "a" : "w");
          if (mam.out == 0)
	    fatal ("cannot write MAM output file `%s'", s);
...
#ifdef MAM
  /* Parameterize some of the MAM action variables.  */
  if (mam.state)
    {
      char *p, *v;
      int f, i, j, k;
      ...
      f = warn_undefined_variables_flag;
      warn_undefined_variables_flag = 0;
      for (i = k = 0; param[i].old_name; i++)
        {
  	if (*param[i].old_name == '\0')
  	  {
  	    if (!never_flag || k++ != 0)
  	      break;
  	    continue;
  	...
  	if (p == 0 || *p != '\0')
  	  {
  	    sprintf (buf, "$(%s)", param[i].old_name);
              p = variable_expand (buf);
  	    if (*p == '\0')
  	      {
  	        p = param[i].def_value;
  	        if (p == 0)
  	...
  	if (v == 0)
  	  v = param[i].old_name;
  	for (j = 0; j < i && strcmp(v, param[j].new_name ? param[j].new_name : param[j].old_name); j++);
  	if (j >= i)
            fprintf (mam.out, "%ssetv %s %s\n", mam.label, v, p);
  	sprintf (buf, "$${%s}", v);
          define_variable (param[i].old_name, strlen(param[i].old_name),
  			 buf, o_override, 1);
        }
      warn_undefined_variables_flag = f;
    }
