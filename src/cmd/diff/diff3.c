...
"[-?\n@(#)diff3 (GNU diffutils " VERSION ") " RELEASE "\n]"
#ifdef USAGE_LICENSE
USAGE_LICENSE
#endif
"[+NAME?diff3 - find differences between three files]"
"[+DESCRIPTION?The \bdiff3\b command compares three files and outputs"
"	descriptions of their differences.]"
"[+?The files to compare are \amine\a, \aolder\a, and \ayours\a. At most one"
"	of these three file names may be \b-\b, corresponding to the"
"	standard input.]"

"[a:text|ascii:?Treat all files as text and compare them line-by-line,"
"	even if they do not appear to be text.]"
"[A:show-all?Incorporate all unmerged changes from \aolder\a to \ayours\a"
"	into \amine\a, surrounding all overlapping changes with bracket lines.]"
"[e:ed?Generate an \bed\b(1) script that incorporates all the changes from"
"	\aolder\a to \ayours\a into \amine\a.]"
"[E:show-overlap?Like \b--ed\b, except that overlapping changes are bracketed.]"
"[L:label?Use \alabel\a instead of the file name.]:[label]"
"[m:merge?Apply the edit script to the first file and send the result to"
"	the standard output. Unlike piping the output from \bdiff3\b to"
"	\bed\b, this works even for binary files and incomplete lines."
"	\b--show-all\b is assumed if no edit script option is specified.]"
"[T:initial-tab?Make tabs line up by prepending a tab to the output.]"
"[x:overlap-only?Like \b--ed\b, except output only the overlapping changes.]"
"[3:easy-only?Like \b--ed\b, except output only the nonoverlapping changes.]"
"[i:compatibility?Generate \bw\b and \bq\b commands at the end of the"
"	\bed\b(1) script for System V compatibility.]"
...
"	[+2?An error occurred.]"
"}"
"[+SEE ALSO?\bcmp\b(1), \bcomm\b(1), \bdiff\b(1), \bed\b(1), \bpatch\b(1),"
"	\bsdiff\b(1)]"
;
