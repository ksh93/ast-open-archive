...
"[-?\n@(#)$Id: codex (AT&T Labs Research) 2004-01-26 $\n]"
USAGE_LICENSE
...
"	are equivalent. For example, \buuencode-base64-text\b specifies the"
"	\buuencode\b method with the \bbase64\b and \btext\b options, and"
...
main(int argc, register char** argv)
{
	...
	for (;;)
	{
		...
		case 'e':
			...
			continue;
		case 'i':
			if (!(buf = sfreserve(ip, CODEX_IDENT, 1)) && !(buf = sfreserve(ip, sfvalue(ip), 1)))
