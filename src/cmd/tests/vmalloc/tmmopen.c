...
tmain()
{
	...
	{	ppid = getpid();
		...
		if(!(shm = vmmopen(shmfile, 1, MAPSIZE)) )
			terror("Parent[pid=%d]: %s: Can't create shm region in parent process", ppid, shmfile);
		tinfo("Parent[pid=%d]: %s: shm region opened", ppid, shmfile);

		tinfo("Parent[pid=%d]: %s: opening map region", ppid, mapfile);
		if(!(map = vmmopen(mapfile, -1, MAPSIZE)) )
			terror("Parent[pid=%d]: %s: Can't create map region in parent process", ppid, mapfile);
