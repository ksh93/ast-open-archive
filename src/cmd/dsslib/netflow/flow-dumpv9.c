...
dumpv9ident(Dssfile_t* file, void* buf, size_t n, Dssdisc_t* disc)
{
	...
	return n >= 20 && BE2(b) == 9 && (n = BE2(b)) >= 1 && n <= 30;
}
		...
		for (;;)
		{
			while (!pp->count--)
			{
				...
				if (file->dss->flags & DSS_DEBUG)
					sfprintf(sfstderr, "header version %d size %d flowsets %d\n", rp->version, 20, pp->count);
			...
			{
				if (!(file->dss->flags & DSS_QUIET) && disc->errorf)
					(*disc->errorf)(NiL, disc, 1, "%s%d: option template definition ignored", cxlocation(file->dss->cx, record));
				file->count++;
			}
			else
			{
