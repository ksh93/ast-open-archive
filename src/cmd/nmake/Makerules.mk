...
.ID. = "@(#)Makerules (AT&T Research) 2000-02-02"
...
.BIND.-l% : .FUNCTION
	...
	if "$(-mam:N=static*,port*)" && ! .BIND.REAL.
		...
		V := ${mam_lib$(B)}
		if "$(%:A=.DONTCARE)" && "$(A:N=/*)"
			print -um setv mam_lib$(B) $(.MAMACTION. $("`")(set -; cd /tmp; echo 'main(){return(0);}' > x.${!-$$}.c; $(CC) $(CCFLAGS) -o x.${!-$$}.x x.${!-$$}.c $(%) >/dev/null 2>&1 && echo ' $(%)'; rm -f x.${!-$$}.[cox])$("`"))
			return + $(V)
	...
	if "$(CC.DIALECT:N=DYNAMIC)" && ( "$(CCLDFLAGS:N=$(CC.DYNAMIC))" || ! "$(CCLDFLAGS:N=$(CC.STATIC))" )
		if "$(CC.SUFFIX.DYNAMIC)" && "$(CC.SUFFIX.SHARED)"
			if ( T = "$(*$(B):N=*$(CC.SUFFIX.DYNAMIC):B:S=$(CC.SUFFIX.SHARED):A=.TARGET)" )
				return $(T)
			end
			if ( T = "$(*$(B):N=-l$(B):/-l//)" )
				T := $(T)$($(B).VERSION:/[^0-9]//G)
...
.BIND.$"{"mam_lib%"}" : .FUNCTION
	if "$(-mam:N=static*,port*)"
		...
		if ( T = "$(T:T=F)" )
			if "$(CC.STDLIB:V:N=$(T:P=D))"
				print -um setv mam_lib$(L) -l$(L)
			else
				print -um setv mam_lib$(L) $(.MAMACTION. ${mam_cc_L+-l$(L)}${mam_cc_L-$(T)})
			end
			return - $(T)
		else
			-l$(L) : .DONTCARE
...
.REQUIRE.-l% : .FUNCTION
	...
	if ! ( T = "$(%:/-l\(.*\)/\1.req/:T=F)" )
		...
		if "$(-mam:N=static*,port*)"
			print -um setv mam_lib$(B) $(.MAMACTION. $("`")${mam_cc_L+cat} ${mam_cc_L-sed} ${mam_cc_L-s,$(%),$(LIBDIR)/lib$(B).a,} $(T)$("`"))
			return ${mam_lib$(B)}
		end
...
.ARCLEAN.LIST. : .FUNCTION
	local I V
	if ! .AR.RETAIN
		for I $(***:T=F:T=G:P=L:A!=.ARCHIVE|.TERMINAL:$(arclean))
...
% : %.sh
	...
	?*:*:*|*::*|*:*:$RANDOM)
		;;
	*)	if	ENV= x= $SHELL -n -c '${list[level]} $$(( 1 + $x ))' 2>/dev/null
		then	ENV= $SHELL -n $(>)
		fi
		;;
	esac
...
cc-% : "" .ALWAYS .LOCAL .FORCE .RECURSE.SEMAPHORE
	...
	then	$(-silent:Y%%echo $(.RWD.:?$(.RWD.)/??)$(<:V:Q): >&2%)
		cd $(<:V:Q)
		$(MAKE) --file=$(MAKEFILE) --keepgoing $(-) --errorid=$(<:V:Q) .ATTRIBUTE.$(IFFEGENDIR)/%:.ACCEPT MAKEPATH=..:$(MAKEPATH) $(=:N!=MAKEPATH=*:Q) $(.RECURSE.ARGS.:N!=cc-*:/^install$/cc-install/) $(%:Y!$$(INSTRUMENT_$$(%:/,.*//):@?instrument=$$(%:/,.*//)?CCFLAGS=-$$(%:V:/$$(%:V:N=*~*:?~?,?)/ /G:@/-W\(.\) /-W\1,/G:@Q)?)!!)
	fi
...
.READONLY : .VIRTUAL /* drop this in 2000 */
...
.RECURSE.DIR : .USE .ALWAYS .LOCAL .FORCE .RECURSE.SEMAPHORE
	...
	else	echo $(<): cannot recurse on virtual directory >&2
	fi
...
":INSTALL:" : .MAKE .OPERATOR
	...
	if ! "$(<:V)"
		...
		if "$(<:V:D)"
			$(<:V:D) : .SPECIAL .SCAN.IGNORE .DO.INSTALL.DIR
			.INSTALL : $(<:V:D)
		end
		.INSTALL : $(<:V)
		if "$(@:V)"
...
":INSTALLDIR:" : .MAKE .OPERATOR
	...
	if ! "$(<:V)" || "$(<:V)" == "."
		...
		$(<:V) : .SPECIAL .SCAN.IGNORE .DO.INSTALL.DIR $(>)
		.INSTALL : $(<:V)
	else
		for T $(R)
			eval
...
":LIBRARY:" : .MAKE .OPERATOR .PROBE.INIT
	...
	$(B).VERSION := $(V)
	S := $(>:V:N=[-+]l*:/[-+]l//) $(.PACKAGE.LIBRARIES. $(.PACKAGE.build:A!=.TARGET):/-l//)
	D := $(>:V:N=-L*)
	if S
		if ! "$(S:N=$(B))"
			S := $(B) $(S)
				...
				*?)	echo " $(D)" ;;
				...
				do	case $i in
					...
					*)	if	test ! -f $$(LIBDIR)/lib$i$(CC.SUFFIX.ARCHIVE)
						then	case `{ $$(CC) $$(CCFLAGS) $$(*.SOURCE.%.ARCHIVE:$$(.CC.NOSTDLIB.):P=L:/^/-L/) $$(LDFLAGS) -o 1.$(tmp).x 1.$(tmp)$(CC.SUFFIX.OBJECT) $(D) -l$i 2>&1 || echo '' $x ;} | $(SED) -e 's/[][()+@?]/#/g' || :` in
...
.SHARED.ON. : .FUNCTION
	if ! "$(static)" && ! "$(CC.LIB.DLL:N=broken)" && "$(CC.DLL)" && ! "$(-mam:N=static*,port*)" && ( "$(.SHARED.FLAGS.:@N=* $(CC.DLL) *)" || "$(.SHARED.FLAGS.:V:@N=* $\(CC.DLL\) *)" )
		return 1
	end
...
.SHARED. : .FUNCTION
	...
	if "$(.SHARED.ON.)"
		...
		$(S) : .SHARED.o lib$(%:O=2)$(CC.LIB.TYPE:O=1)$(CC.SUFFIX.ARCHIVE) $$(.SHARED.BIND. $(L))
		if ! "$(.INSTALL.$(S))"
			$$(LIBDIR) :INSTALLDIR: $(S)
...
.SHARED.lib : .USE $$(LDLIBRARIES)
	...
	$(LD) $(LDFLAGS) $(CCFLAGS:N=-[gG]*) $(CC.SHARED) -o $(<:O=1:B) $(.SHARED.REF.lib $(*))

/*
 * NOTE: mvs.390 env var LIBPATH must contain DLLDIR
 *	 win32.* got there first with DLLDIR=$(BINDIR)
 *	 so the default behavior forces mvs.390 to have bins
 *	 in LIBPATH; you can either export DLLDIR=$INSTALLROOT/lib
 *	 or export LIBPATH=$PATH
 */

.SHARED.x : .USE
	$(CC) $(LDFLAGS) $(CCFLAGS:N=-[gG]*) $(CC.SHARED) -o $(<:O=1:B:S) $(*)
...
":READONLY:" : .MAKE .OPERATOR .PROBE.INIT
	...
	for T $(>)
		if "$(@.READONLY$(T:S):V)"
			...
			error 1 READONLY not implemented for suffix $(T:S)
		end
	end
...
":WORKAROUND:" : .MAKE .OPERATOR
	...
	$(<) : .TERMINAL
		...
		$$(>) : .SCAN.NULL
		$$(<) : -TERMINAL $$(>)
			$(@:V:@?$$(>:V:@Y%%test "" != "$$$$(.WORKAROUND.COVERED.)" && $$$$(CP) $$$$(.WORKAROUND.COVERED.) $$$$(<)$$("\n")%)$$(@:V)?$$(>:@Y%$$$$(CP) $$$$(*)%: >%) $$$(<)?)
		end
	end
...
.MAKEINIT : .MAKE .VIRTUAL .FORCE
	...
	if T1 = "$(.PACKAGE.build:A!=.TARGET)"
		...
		for T3 $(T1)
			...
			if ( !I || !L )
				if ! ( T1 = "$(PACKAGE_$(T3))" )
					T4 := $(PACKAGE:/:/ /G) $(Y) $(PATH:/:/ /G:D) $(OPTDIRS:/:/ /G) $(OPTDIRS:/:/ /G:C%$%/$(T3)%)
					V =
					...
					if ! "$(T1)"
						if ! ( T1 = "$(T4:C%$%/include/$(T3)%:P=X:O=1:D:D)" )
							T1 := $(T4:N=*/$(T3):C%$%/include%:P=X:O=1:D)
						end
					end
					eval
					PACKAGE_$(T3) = $(T1:P=C)
			...
			T4 =
			if "$(-mam:N=static*)"
				if "$(INCLUDEDIR:B:N=$(T3))" || "$(INCLUDEDIR:D:N=$(INSTALLROOT))"
					T4 += P
				end
				if I && "$(I:D)" == "$(L:D)"
					print -um setv PACKAGE_$(T3) $(T4:N=P:?${INSTALLROOT}?$(I:D)?)
					print -um setv PACKAGE_$(T3)_INCLUDE ${PACKAGE_$(T3)}/$(I:B:S)
	...
	T3 =
	if "$(CC.REPOSITORY)"
		T3 += $(T5:V) $$(CCFLAGS:N=-[D]*) $$(.PTR.PREREQS:A=.SCAN.c:@?$$$(*.SOURCE.%.LCL.INCLUDE:I=$$$$(.PTR.PREREQS:A=.LCL.INCLUDE:P=D):/^/-I/) -I- $$$(*.SOURCE.%.LCL.INCLUDE:I=$$$$(.PTR.PREREQS:A=.LCL.INCLUDE|.STD.INCLUDE:P=D):$(.CC.NOSTDINCLUDE.):/^/-I/)??) $$(&.PTR.CHECK:T=D)
	...
	USAGE_LICENSE == "$(LICENSEFILES:/:/ /G:T=F:O=1:P=W=$(LICENSE),type=usage)$(DICTIONARY:Y%[--dictionary?$(DICTIONARY)]%%)"

	/*
	 * map unknown command line targets to common actions
	 */

	for T3 .ALL .ARGS .MAIN
		T2 := $(~$(T3):V)
		eval
		.ORIGINAL$(T3). = $(T2:V)
		end
 ...
 *	$(.ORIGINAL.ALL.) original .ALL prereqs
...
.CC.LIB.DLL.symbol : .FUNCTION
	...
	set $(J)
	return $(F)$(CC.SUFFIX.OBJECT) $(%) -lm -lc
...
.CLOBBER : .CLEARARGS .ONOBJECT .NOSTATEFILE
	$(IGNORE) $(RM) $(RMFLAGS) $(.CLOBBER.:T=F:P=L) $(.BUILT.:A=.REGULAR)
...
.LIST.PACKAGE.SOURCE : .MAKE
	...
	for I $(.MANIFEST.:P=A)
		if I == "*.[chly]"
			...
			print ^^$(PROTO) $(PROTOFLAGS) -dpS $(I)^$(I)
		else
			print ^^^$(I)
		end
	end
...
.OFFICIAL : .ONOBJECT
	$(".RWD.:=":R)$(*.VIEW:O=2:@?$$(.MANIFEST.:P=L:N!=/*:C@.*@{ echo "=== & $(_hosttype_) $$(&:T=R:F=%T) ==="; $$(DIFF) $$(DIFFFLAGS) $$(*.VIEW:O=2)/& & || true; } >> $(official_out:D=$$(*.VIEW:O=2):B:S); $$(MV) & $$(*.VIEW:O=2)/&;@)?: no lower view?)
...
.MAM.INIT : .MAKE .VIRTUAL .FORCE .AFTER
	...
	.ARCLEAN : .NULL
	.DO.INSTALL :
		$(MV) $(<) $(<).old 2>/dev/null || true; $(CP) $(*) $(<) $(.DO.INSTALL.OPT.)
	for T .READONLY.*
